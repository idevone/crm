...
class FinancialGridView extends Widget
{
    public $date;
        ...
        switch ($this->date) {
            case 'all':
                $dataProvider = new ActiveDataProvider([
                    'query' => \app\models\FinancialRecordForm::find(),
                ]);
                break;
            case 'today':
                $dataProvider = new ActiveDataProvider([
                    'query' => \app\models\FinancialRecordForm::find()->where(['created_at' => date('Y-m-d')]),
                ]);
                break;
            case 'yesterday':
                $dataProvider = new ActiveDataProvider([
                    'query' => \app\models\FinancialRecordForm::find()->where(['created_at' => date('Y-m-d', strtotime('-1 day'))]),
                ]);
                break;
            case 'week':
                $dataProvider = new ActiveDataProvider([
                    'query' => \app\models\FinancialRecordForm::find()->where(['between', 'created_at', date('Y-m-d', strtotime('-7 day')), date('Y-m-d')]),
                ]);
                break;
            case 'month':
                $dataProvider = new ActiveDataProvider([
                    'query' => \app\models\FinancialRecordForm::find()->where(['between', 'created_at', date('Y-m-d', strtotime('-30 day')), date('Y-m-d')]),
                ]);
                break;
            default:
                $dataProvider = new ActiveDataProvider([
